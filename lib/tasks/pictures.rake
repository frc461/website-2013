namespace :photo do
	...
	task :import, [:path, :id] => :environment do |t, args|
		...
		Dir.glob(args[:path] + '/*.jpg').entries.each do |e|
			...
			req.set_form_data('album_id' => params[:id],
			                  'image_file_name' => remote_file_name,
			                  'image_content_type' => remote_content_type,
			                  'image_file_size' => remote_file_size,
			                  'image_updated_at' => remote_updated_at)
